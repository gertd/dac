...
//
//    So for the wc.exe program e want this:
//
//    using System;
//    using Utilities;
//
...
namespace CommandLine
{
    ...
    public sealed class Parser
    {
        ...
        private static string LongName(ArgumentAttribute attribute, PropertyInfo field)
        {
            return (attribute == null || attribute.DefaultLongName) ? field.Name : attribute.LongName;
        }

        private static string ShortName(ArgumentAttribute attribute, PropertyInfo field)
        {
            if (attribute is DefaultArgumentAttribute)
                return null;
        ...
        private static string HelpText(ArgumentAttribute attribute, PropertyInfo field)
        {
            if (attribute == null)
                return null;
            else
        ...
        private static object DefaultValue(ArgumentAttribute attribute, PropertyInfo field)
        {
            return (attribute == null || !attribute.HasDefaultValue) ? null : attribute.DefaultValue;
        }

        private static Type ElementType(PropertyInfo field)
        {
            if (IsCollectionType(field.PropertyType))
                return field.PropertyType.GetElementType();
            else
                return null;
        }

        private static ArgumentType Flags(ArgumentAttribute attribute, PropertyInfo field)
        {
            if (attribute != null)
                return attribute.Type;
            else if (IsCollectionType(field.PropertyType))
        ...
        {
            public Argument(ArgumentAttribute attribute, PropertyInfo prop, ErrorReporter reporter)
            {
                this.longName = Parser.LongName(attribute, prop);
                this.explicitShortName = Parser.ExplicitShortName(attribute);
                this.shortName = Parser.ShortName(attribute, prop);
                this.hasHelpText = Parser.HasHelpText(attribute);
                this.helpText = Parser.HelpText(attribute, prop);
                this.defaultValue = Parser.DefaultValue(attribute, prop);
                this.elementType = ElementType(prop);
                this.flags = Flags(attribute, prop);
                this.prop = prop;
                ...
                Debug.Assert(!this.HasDefaultValue || (this.defaultValue.GetType() == prop.PropertyType), "Type of default value must match field type");
            }
            ...
            public bool Finish(object destination)
            {
                ...
                {
                    this.prop.SetValue(destination, this.DefaultValue, null);
                }
                if (this.IsCollection)
                {
                    this.prop.SetValue(destination, this.collectionValues.ToArray(this.elementType), null);
                }
            ...
            public bool SetValue(string value, object destination)
            {
                ...
                {
                    this.prop.SetValue(destination, newValue, null);
                }
